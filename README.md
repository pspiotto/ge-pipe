...
| `agg_flip_opportunities` | view | Current best flips across **all price tiers**: **real-time** (`/latest`) prices for the spread. Loose liquidity floor (traded in the last hour); per-lane gating is the consumer's job via `volume_per_hour`, `avg_volume_per_hour_24h`, `est_hours_to_fill_limit` (time-to-fill), `mid_volatility_1h`/`mid_volatility_pct_1h` (spread-sizing), `is_high_value` (high-value low-volume lane), and `is_stale`/`price_age_minutes` (stale/phantom-spread screen). Consumer guide: [`docs/agg_flip_changes.md`](docs/agg_flip_changes.md). |
...
**Write contract:** the bot appends one row per fill/cancel event over a **separate, short-lived autocommit connection**, kept entirely off its read path (which is `setReadOnly(true)`), so fill-writes can never lock or stall opportunity fetches. `event_id` and `recorded_at` are assigned by the DB; the bot supplies the rest. `leg` ∈ {`buy`,`sell`}; `event` ∈ {`filled`,`partial`,`cancelled_deadline`,`cancelled_pricemove`,`cancelled_manual`} (validated by dbt `accepted_values` tests, not a DB `CHECK`, so a stray value surfaces as a failing test rather than a rejected write). Full bot-side integration guide: [`docs/realized_fills_bot_integration.md`](docs/realized_fills_bot_integration.md) (field→source mapping also in `init_db/004_create_realized_fills.sql`).
